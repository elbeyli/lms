<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center space-x-4">
            <a href="{{ route('courses.show', $course) }}" class="text-gray-500 hover:text-gray-700">
            ...
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">Update {{ $course->name }} details</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-2xl mx-auto">
        <div class="card">
            <div class="card-body">
                <form id="courseForm" action="{{ route('courses.update', $course) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-6">
                        ...
                        @enderror
                        <div id="subjectPreview" class="mt-2 flex items-center text-sm text-gray-600">
                            <div id="subjectColor" class="w-3 h-3 rounded-full mr-2" style="background-color: {{ $course->subject->color }}"></div>
                            <span id="subjectName">{{ $course->subject->name }}</span>
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            <div class="flex items-center space-x-3">
                                <input type="range" 
                                       ...
                                       min="1" 
                                       max="10" 
                                       value="{{ old('priority', $course->priority) }}" 
                                       class="flex-1"
                                       oninput="updatePriorityDisplay(this.value)">
                                <div class="flex items-center space-x-2">
                                    <span id="priorityValue" class="text-sm font-medium text-gray-700">{{ old('priority', $course->priority) }}</span>
                            ...
                            <div id="priorityLabel" class="text-xs mt-1">
                                ...
                                @endphp
                                <span class="priority-label text-gray-500">{{ $priorityLabels[old('priority', $course->priority)] }}</span>
                            </div>
                            @error('priority')
                                <p class="form-error">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="mb-6">
                        ...
                        <div class="flex items-center">
                            <input type="datetime-local" 
                                   id="deadline" 
                                   name="deadline" 
                                   value="{{ old('deadline', $course->deadline?->format('Y-m-d\TH:i')) }}" 
                                   class="form-input @error('deadline') border-red-500 @enderror"
                                   min="{{ date('Y-m-d\TH:i') }}">
                        </div>
                        <p class="text-xs text-gray-500 mt-1">Optional deadline for completion</p>
                        @error('deadline')
                            <p class="form-error">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="flex items-center space-x-3">
                            <input type="range" 
                                   ...
                                   min="0" 
                                   max="100" 
                                   value="{{ old('progress_percentage', $course->progress_percentage) }}" 
                                   class="flex-1"
                                   oninput="updateProgressDisplay(this.value)">
                            <div class="flex items-center space-x-2">
                                <span id="progressValue" class="text-sm font-medium text-gray-700">{{ old('progress_percentage', $course->progress_percentage) }}</span>
                                <span class="text-sm text-gray-500">%</span>
                            </div>
                        </div>
                        <div class="mt-2">
                            <div class="w-full bg-gray-200 rounded-full h-2">
                                <div id="progressBar" class="bg-blue-600 h-2 rounded-full transition-all duration-300" style="width: {{ old('progress_percentage', $course->progress_percentage) }}%"></div>
                            </div>
                        </div>
                        <p class="text-xs text-gray-500 mt-1">Update your current progress on this course</p>
                    ...
                    <div class="flex items-center justify-end space-x-3 pt-6 border-t border-gray-200">
                        <a href="{{ route('courses.show', $course) }}" class="btn-secondary">Cancel</a>
        ...
        <div class="card mt-8 border-red-200">
            ...
            <div class="card-body">
                <div class="flex items-center justify-between">
                    <div>
                        <h4 class="text-sm font-medium text-gray-900">Delete this course</h4>
                        <p class="text-sm text-gray-600">This will permanently delete the course and all associated topics.</p>
                    </div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white rounded-lg max-w-lg w-full">
                <div class="p-6">
                    ...
                    <h3 class="text-lg font-medium text-gray-900 mb-2 text-center">Delete "{{ $course->name }}"</h3>
